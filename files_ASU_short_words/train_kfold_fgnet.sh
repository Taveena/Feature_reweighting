...
declare -a arr=("eegnetfusion" "mieegnet") # eegnet, eegnetfusion, mieegnet, shallow, deep, tsseffnet, fgnet, channel_gated_final temporal_gated_final, temporal_gated_final, fgnet_msfnot. fgnet_3abl, fgnet_5abl, conformer, fgnet_transformer, DeformConvNeXt_orig
...
do
	...
	do	
		...
		do
			...
			CUDA_VISIBLE_DEVICES=0 python train_kfold_fgnet.py --no_epochs $NO_EPOCHS --n_classes $N_CLASSES --batch_size $BATCH_SIZE --train_list $TRAIN_LIST --test_list $TEST_LIST --val_list $VAL_LIST --model_name $MODEL_NAME --fold $k --lr $LR --channel $CHANNEL --window $WINDOW --log_file $LOG_FILE --model_save_path $MODEL_SAVE_PATH --subject $j
