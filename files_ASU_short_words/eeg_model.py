...
class EEGNet_fusion(nn.Module):
    '''Data shape = (trials, kernels, channels, samples), which for the
        input layer, will be (trials, 1, channels, samples).'''
    def __init__(self,nb_classes,Chans=64, Samples=128, n_size=224):
...
class Inception(nn.Module):
    def __init__(self,Chans=64, Samples=128,
        ...
        self.conv4_1 = nn.Conv2d(in_channels=Chans,out_channels=64,kernel_size =(1,1),padding=(0,0), stride=(1,2),bias=False)
...
if __name__=="__main__":
     ...
     batch_size = 10
     chans =64
     #net=EEGNet_v2(4, Chans=22, Samples=1000).cuda()
     #net=EEGNet_fusion(4, Chans=22, Samples=1000).cuda()
     net=MI_EEGNet(4, Chans=22, Samples=1000).cuda()
...
'''
     net = EEGChannelNet()
     print(net.forward(Variable(torch.Tensor(np.random.rand(batch_size, chans, 80)))))    #batchx1xdatapointsxchannels
