...
class SpatialSELayer(nn.Module):
    ...
    def __init__(self, num_channels, Samples, reduction_ratio=2):
        ...
        self.upsample1 = nn.Upsample(scale_factor = (1,5), mode = 'nearest')
        self.avg_pool_block2 = nn.AdaptiveAvgPool2d((1,Samples//10))
        self.upsample2 = nn.Upsample(scale_factor = (1,10), mode = 'nearest')
        self.avg_pool_block3 = nn.AdaptiveAvgPool2d((1,Samples//20)) 
        self.upsample3 = nn.Upsample(scale_factor = (1,20), mode = 'nearest')
...
class sknet_module(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        batch_size, channel, a, b = input_tensor.size()  
        #print('batch_size, channel, a, b', batch_size, channel, a, b)   
...
class Inception(nn.Module):
    ...
    def forward(self, input):
        ...
        '''
        #print('inception module input shape', input.shape)
        block5 = F.elu(self.conv5(input))
...
class EEGNet_fgnet_msfnot(nn.Module):
    '''Data shape = (trials, kernels, channels, samples), which for the
        input layer, will be (trials, 1, channels, samples).'''
    #TODO resolve problems with avg padding when the end of the epoch lost
    #TODO possible solution via padding or AdaptiveAvgPool2d
    def __init__(self,nb_classes, Chans=64, Samples=1000):
        super(EEGNet_fgnet_msfnot,self).__init__()
        self.Chans = Chans
        self.Samples = Samples
...
if __name__=='__main__':
     ...
     net=EEGNet_fgnet_msfnot(1, Chans=64, Samples=80)
     #input=torch.randn(5,1,64,80)
     #output = net(input)
     from torchinfo import summary
     summary(net.cuda(), (5, 1,64, 80))
...
class SpatialChannelSELayer(nn.Module):
    def __init__(self, num_channels, time_stamp):
        """
        :param num_channels: No of input channels
        """
        super(SpatialChannelSELayer, self).__init__()
        self.channelSE=ChannelSSLayer(num_channels) 
        self.spatialSE=SpatialSELayer(num_channels)
    ...
    def forward(self, input_tensor):
        ...
        temporalwise = self.spatialSE(input_tensor).repeat(1,channel, 1,1)
        map1 = input_tensor*channel_wise
        map2 = temporalwise*input_tensor 
        combined = torch.cat([map1, map2],dim=1)
        
        #temporalwise = temporalwise.repeat(1,channel, 1,1)
        #attention_map = channel_wise* temporalwise
        output_tensor= input_tensor* attention_map
        aux_tensor = torch.mean(output_tensor.squeeze(),dim=-1)
        return output_tensor, aux_tensor
'''
...
class SpatialSELayer(nn.Module):
    ...
    def __init__(self, num_channels):
        ...
        """
        super(SpatialSELayer, self).__init__()
        self.conv = nn.Conv2d(num_channels, 1, 1,bias=False)
        #self.conv = nn.Conv2d(num_channels, 1, kernel_size=[1,50], stride=[1,1],bias=False, padding='same')
        self.sigmoid = nn.Softmax(dim=-1)#Sigmoid()#
...
class SpatialChannelSELayer(nn.Module):
    def __init__(self, num_channels, Samples):
        ...
        self.channelSE=ChannelSSLayer(num_channels) 
        self.spatialSE=SpatialSELayer(num_channels, Samples)
        self.value = nn.Conv2d(num_channels, num_channels, 1,bias=False)  #not used for now
        self.combine = nn.Conv2d(2, 1, 1,bias=False)  #not used for now       
    def forward(self, input_tensor):
        """
        :param weights: weights for few shot learning
        :param input_tensor: X, shape = (batch_size, num_channels, H, W)
        ...
        value = F.elu(self.value(input_tensor))   
        batch_size, channel, a, b = value.size()  
        #print('value shape', value.shape, 'batch_size, channel, a, b', batch_size, channel, a, b)   
        channelwise = self.channelSE(input_tensor).repeat(1,1,1,b)
        temporalwise = self.spatialSE(input_tensor).repeat(1,channel, 1,1)
        #print('temporalwise shape', temporalwise.shape, 'channelwise shape', channelwise.shape)
        # version 1 
        #attention_map = (channelwise + temporalwise)/2.0
        #output_tensor= value* attention_map  
        # version 2
        #value = value * channelwise
        #output_tensor = value * temporalwise
        # version 3
        #value_channel = value * channelwise
        #value_temporal = value * temporalwise
        #print(value_channel.shape)
        #output_tensor = F.elu(self.combine(torch.cat([value_channel, value_temporal], axis= 1))) 

        # version 4
        #value_channel = value * channelwise
        #value_temporal = value * temporalwise
        #output_tensor = value_channel*value_temporal

        # version 5
        attention_map = F.sigmoid(self.combine(torch.cat([channelwise.permute(0,2,1,3), temporalwise.permute(0,2,1,3)],axis=1)))
        attention_map = attention_map.permute(0,2,1,3)
        #print(attention_map.shape)
        #tt = torch.cat([channelwise, temporalwise], axis=1)
        #attention_map = F.sigmoid(self.combine() 
        #print(tt.shape)
        output_tensor = value * attention_map
