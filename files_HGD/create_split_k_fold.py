...
write_split='./split_hgd_5fold/'#+subject
read_data='./hgd_data/'+subject

if not os.path.exists(write_split):
	os.mkdir(write_split)
if not os.path.exists(os.path.join(write_split, subject)):
	os.mkdir(os.path.join(write_split, subject))
...
def dump_file_new_5fold(data, per=[0.9,0.875]):
	...
	for i, fold in enumerate(folds):
		...
		test_write_name = os.path.join(write_split, subject, str(i+1)+'_'+'test'+'.txt')
		for instance in test_data:
			string = ','.join(instance)
			with open(test_write_name, 'a') as fp:
				fp.write(string+'\n')

		train_write_name =os.path.join(write_split, subject, str(i+1)+'_'+'train'+'.txt')	
		for instance in train_data:
			string = ','.join(instance)
			with open(train_write_name, 'a') as fp:
				fp.write(string+'\n')

		val_write_name =os.path.join(write_split, subject, str(i+1)+'_'+'val'+'.txt')
		for instance in val_data:
			string = ','.join(instance)
...
for dir in directory:
	...
	for dirName, subdirList, fileList in os.walk(dir):
		...
		#input('halt')
		for fname in fileList:
			complete_data.append([os.path.join(dirName,fname),dirName.split(os.sep)[-2]])
			#print(fname)
