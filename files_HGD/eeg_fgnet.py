...
class EEGNet_fgnet(nn.Module):
    ...
    #TODO resolve problems with avg padding when the end of the epoch lost
    #TODO possible solution via padding or AdaptiveAvgPool2d
    def __init__(self,nb_classes, Chans=64, Samples=1000, moabb=False):
    ...
    def forward(self, input, embedding=False):
        ...
        #print('value of moabb', self.moabb)
        if self.moabb==True:
             out = out.unsqueeze(dim=-1)
             out = F.softmax(out)
        #out = squeeze_final_output(out)
...
if __name__=='__main__':
     net=EEGNet_fgnet(4, Chans=45, Samples=1000)
     input=torch.randn(1, 45, 1000)
     output = net(input)
     from torchinfo import summary
     summary(net.cuda(), (1, 45, 1000))
