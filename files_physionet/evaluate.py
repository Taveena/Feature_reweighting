...
window=1000
n_channels=22
model_name = 'DeepConvNet_orig'   # EEGNet, EEGNet_fusion, MI_EEGNet, DeepConvNet_orig, shallowconvnet_repeated, TS_SEFFNet, temporal_gated_final, channel_gated_final, fgnet, fgnet_msfnot

#model_path='weights/EEGNet_fusion/A01/1/'
#model_path='weights/MI_EEGNet/A01/1/'
#model_path='weights/proposed_channel_gated/A01/1/'
#model_path='./weights/TS_SEFFNet/A01/1/'
#model_path='./weights/proposed_aux/A01/1/'

classes=['L Hand', 'R Hand', 'Tongue', 'B Feet']
...
def evaluate(model, loader):
	...
	with torch.no_grad():
		for i, (inputs, labels) in enumerate(loader): 
			...
			if model_name in ['tsseffnet_repeated', 'TS_SEFFNet']:
...
kappa=[]
for j in range(1,10):
	print('Subject A0'+str(j))
	for i in range(1,11):
		model_path='weights/'+model_name+'/A0'+str(j)+'/'+str(i)+'/'
		data_path='./split_bci/A0'+str(j)+'/A0'+str(j)+'_'+str(i)+'_test.txt'
		...
		#net = ShallowConvNet(n_classes=n_classes, input_shape=[batch_size,1,n_channels,window], F1=40, T1=25, F2=40, P1_T=75, P1_S=15, drop_out=0.5, last_dim=2440).cuda()
		#net = EEGNet_proposed_temporal_gated_final(n_classes,Chans=channel, Samples=window).cuda()
		#net = DeepConvNet(n_classes, input_shape=[batch_size,1,channel,window], first_conv_length=10, block_out_channels=[25,25,50,100,200], pool_size=3, last_dim=1400).cuda()
