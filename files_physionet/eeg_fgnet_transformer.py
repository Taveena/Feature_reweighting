...
class ChannelSSLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        # channel excitation
        tensor_a = self.elu(self.fc1_1(squeeze_tensor1))
        tensor_b = self.elu(self.fc1_2(squeeze_tensor2))
        tensor_c = self.elu(self.fc1_3(squeeze_tensor3))
        tensor_d = self.elu(self.fc1_4(squeeze_tensor4))
        ...
        fc_out_2 = self.elu(self.fc2(fc_out_1))
...
class Inception(nn.Module):
    def __init__(self,Chans=64, Samples=128,
        ...
        self.sknet = nn.Sequential(
                            sknet_module(Chans//2,Samples),
...
class MI_EEGNet_predict(nn.Module):
    ...
    def forward(self,input): #,aux):
        ...
        features = self.dropout(self.avg_pool_block(input))
...
class EEGNet_fgnet_transformer(nn.Module):
    '''Data shape = (trials, kernels, channels, samples), which for the
        input layer, will be (trials, 1, channels, samples).'''
    #TODO resolve problems with avg padding when the end of the epoch lost
    #TODO possible solution via padding or AdaptiveAvgPool2d
    def __init__(self,nb_classes, Chans=64, Samples=1000, kernel=128, emb_size=128, att_depth=1, att_heads=8, att_drop=0.5, fc_dim=2560,):
        super(EEGNet_fgnet_transformer,self).__init__()
        self.Chans = Chans
        self.Samples = Samples
        ...
        self.transformerblock = TransformerEncoder(att_depth, emb_size, att_heads, att_drop)
        self.prediction = MI_EEGNet_predict(nb_classes=nb_classes, Chans=256, Samples=Samples//4)
        self.classification = ClassificationHead(emb_size, fc_dim, n_classes=nb_classes)
    ...
    def forward(self, input, embedding=False):
        ...
        out = self.transformerblock(out)  #, aux
        #print('out.shape inception', out.shape) #, 'aux shape', len(aux))
        out, features = self.prediction(out)
        #out, features = self.classification(out)  #  , aux            #, aux1, aux2 
...
if __name__=='__main__':
     net=EEGNet_fgnet_transformer(1, Chans=22, Samples=1000)
