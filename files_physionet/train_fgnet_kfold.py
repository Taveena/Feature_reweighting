...
parser.add_argument('--log_file', type=str, default='log_files_physionet_5fold/', 
...
parser.add_argument('--train_list', type=str, default='./splits/split_cat4/imagined/train_split_cat4.txt', 
                    help='the log file storage path')
parser.add_argument('--test_list', type=str, default='./splits/split_cat4/imagined/test_split_cat4.txt', 
                    help='the log file storage path')
parser.add_argument('--val_list', type=str, default='./splits/split_cat4/imagined/val_split_cat4.txt', 
                    help='the log file storage path')
...
if not os.path.exists(os.path.join(model_save_path,model_name,str(fold))):
	os.mkdir(os.path.join(model_save_path,model_name,str(fold)))
...
log_file_path = os.path.join(log_file, model_name,'log_'+str(fold)+'.txt')
...
if resume_train:
	...
	checkpoint = torch.load(os.path.join(model_save_path,model_name,str(fold),'current.pth'))
...
for epoch in range(no_epoch):  # loop over the dataset multiple times
	...
	torch.save({
            ...
            'best_test_acc' :best_test_acc }, os.path.join(model_save_path,model_name,str(fold),'current.pth'))
	...
	if val_loss<best_val_loss:
		...
		torch.save(net.state_dict(), os.path.join(model_save_path,model_name,str(fold),'best_val_loss_weight.pth'))
		counter= counter+1
	...
	if test_metric[0]>best_test_acc:
		...
		torch.save(net.state_dict(), os.path.join(model_save_path,model_name,str(fold),'best_test_acc_weight.pth'))
